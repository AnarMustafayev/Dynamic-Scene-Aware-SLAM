    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL))
{
    ...
    // ── YOLODetector: optional semantic dynamic-object filter ──────────────
    // Read model path from the settings YAML.
    // Key: YOLODetector.ModelPath   (leave empty / absent to disable)
    try {
        cv::FileStorage fSettings(
            strSettingPath.empty() ? "" : strSettingPath,
            cv::FileStorage::READ);
        std::string yolo_path;
        if (fSettings.isOpened()) {
            cv::FileNode node = fSettings["YOLODetector.ModelPath"];
            if (!node.empty())
                node >> yolo_path;
        }
        if (!yolo_path.empty()) {
            ...
            if (fSettings.isOpened()) {
                if (!fSettings["YOLODetector.InputSize"].empty())
                    fSettings["YOLODetector.InputSize"]   >> input_size;
                if (!fSettings["YOLODetector.ConfThreshold"].empty())
                    fSettings["YOLODetector.ConfThreshold"] >> conf_thresh;
                if (!fSettings["YOLODetector.MaskThreshold"].empty())
                    fSettings["YOLODetector.MaskThreshold"] >> mask_thresh;
            }
            mpYOLODetector = std::make_shared<YOLODetector>(
                yolo_path, input_size, conf_thresh, mask_thresh);
            mpYOLODetector->Start();
        } else {
            std::cout << "[Tracking] YOLODetector.ModelPath not set — "
                         "semantic filter disabled." << std::endl;
        }
    } catch (const std::exception& e) {
        std::cerr << "[Tracking] YOLODetector init failed (continuing without it): "
                  << e.what() << std::endl;
        mpYOLODetector.reset();
    }
    // ── End YOLODetector init ──────────────────────────────────────────────

    // ── DynamicFilter init ─────────────────────────────────────────────────
    // Cascade decision tree: Epipolar → YOLO → Flow (Step 5).
    // Epipolar threshold = 3.84 (chi² 95% CI, 1 DOF).
    mpDynamicFilter = std::make_shared<DynamicFilter>(3.84f);
    std::cout << "[Tracking] DynamicFilter initialized (epi_thresh=3.84 | "
                 "cascade: Epipolar→YOLO→Flow)" << std::endl;
    // ── End DynamicFilter init ─────────────────────────────────────────────
}
...
void Tracking::Track()
{
    ...
    {
        // Push frame to YOLO async thread (non-blocking, before tracking)
        if (mpYOLODetector)
            mpYOLODetector->PushFrame(mImGray);
    ...
    else
    {
        ...
        // Push frame to YOLO async thread (non-blocking)
        if (mpYOLODetector)
            mpYOLODetector->PushFrame(mImGray);
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // ── Unified dynamic-point filter ──────────────────────────────────────────
    // Three stages run as one unit inside mpDynamicFilter->FilterFrame():
    //   Stage 1: YOLO semantic mask  (humans / vehicles)
    //   Stage 2: Optical-flow mask   (placeholder — Step 5)
    //   Stage 3: Epipolar Sampson    (geometric, ranked, min-keep guaranteed)
    //
    // F is only valid when velocity has a non-degenerate translation and we are
    // on the pure-vision path (non-IMU).  Pass empty Mat otherwise so the
    // epipolar stage is skipped gracefully.
    {
        cv::Mat F_mat;   // fundamental matrix — empty = skip epipolar stage

        if (mbVelocity && !mpAtlas->isImuInitialized())
        {
            Eigen::Vector3f t_eig = mVelocity.translation();
            if (t_eig.norm() > 0.001f)    // guard: skip epipolar on pure rotation
            {                             //        (< 1mm translation → degenerate F)
                Eigen::Matrix3f R_eig = mVelocity.rotationMatrix();
                ...
                F_mat = mEpipolarChecker.ComputeFundamentalMatrix(R, t, mK);
            }
        }

        // Collect masks from async sources (non-blocking; empty if not ready)
        cv::Mat yolo_mask = (mpYOLODetector && mpYOLODetector->HasMask())
                            ? mpYOLODetector->GetLatestMask()
                            : cv::Mat();

        cv::Mat flow_mask;   // Step 5 placeholder — always empty for now
        // cv::Mat flow_mask = (mpFlowChecker && mpFlowChecker->HasMask())
        //                     ? mpFlowChecker->GetLatestMask() : cv::Mat();

        mpDynamicFilter->FilterFrame(
            mCurrentFrame,
            mLastFrame,
            F_mat,
            yolo_mask,
    ...
    // ── End unified dynamic-point filter ──────────────────────────────────────
