...
void System::Shutdown()
{
    ...
    // This is critical: without waiting, SaveKeyFrameTrajectory* races with
    // LocalMapper/LoopCloser deleting KeyFrames → pure virtual method crash.
    while(!mpLocalMapper->isFinished() || !mpLoopCloser->isFinished())
    {
        if(!mpLocalMapper->isFinished())
            cout << "Waiting for LocalMapper to finish..." << endl;
        if(!mpLoopCloser->isFinished())
            cout << "Waiting for LoopCloser to finish..." << endl;
        usleep(5000);
    }
