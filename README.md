...
### Extended for Teknofest Aerial Dataset
...
### `datasets/aerial/prepare_teknofest.py`
Utility script that converts the raw Teknofest dataset folder into **TUM RGB-D format** (`rgb.txt`, `groundtruth.txt`) required by ORB-SLAM3's monocular examples:
```bash
python datasets/aerial/prepare_teknofest.py \
    --input  datasets/aerial/images/ \
    --gt     datasets/aerial/gps_groundtruth.csv \
    --output datasets/aerial/
```
...
datasets/
└── aerial/
    ├── images/          ← raw JPEG frames (3840×2160, 7.5 fps)
    ├── gps_groundtruth.csv
    ├── groundtruth.txt  ← generated by prepare_teknofest.py
    └── rgb.txt          ← generated by prepare_teknofest.py
```
...
|---|---|---|
| **High Z-axis error** | Top-down aerial view → planar degeneracy → monocular depth estimation fails for altitude | `thFarPoints: 30.0` prunes runaway depth estimates; Z smoothed in visualization |
